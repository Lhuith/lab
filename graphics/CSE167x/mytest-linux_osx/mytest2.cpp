...
void display(void)
{
	...
	if (DEMO > 0) {
		// We now maintain a stack for the modelview matrices. Changes made to the stack after pushing are discarded once it is popped.
	...
	// Draw a teapot. GLUT's teapot uses the old OpenGL, which is not usable in the modern OpenGL context, therefore a 3D model of the teapot is loaded (in the init() func) and drawn instead.
	if (DEMO >= 3) {
...
int main(int argc, char** argv)
{
  ...
  //GLew not needed on OSX due to its unique headers
#ifndef __APPLE__
	GLenum err = glewInit();
	if (GLEW_OK != err) {
