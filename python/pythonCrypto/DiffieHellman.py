...
def get_generator(p, q, lim):
    for g in range(2, lim):
        isGen, value, i = is_generator2(g, q)
        if isGen:
...
def printResults(g, v, i, isGen):
    ...
    #print(isGen, "g-", g, "g^i mod p -", v, "i -", i)

# public (green)
p = 2027 #get_prime(2027)
q = math.floor((p - 1)/2)
print(q)

g = get_generator(p, q, 100)
print(g, p)

# Alice
a = 123 #random.randrange(0, p)
g_a = (g**a) % p
# Alice sends this
print("Alice sends g_a: ", g_a)

b = 456 #random.randrange(0, p)
g_b = (g**b) % p
# Bob sends this
